...
class Application(tk.Tk):
	...
	def createWidgets(self):
		...
		tk.Label(text="Weights: ", font=('Comic Sans MS', 12)).grid(row=10, column=0)
		self.weight_text = tk.Text(self, height=4, width=30, font=('Comic Sans MS', 10))
		self.weight_text.grid(row=10, column=1)
		"""
	...
	def draw_picutre(self):
		...
		Train_acc, x_train, y_train, Test_acc, x_test, y_test, weights = process(self.File, epochs, learning_rate)
		...
		for i in range(len(weights)):
			for j in range(len(weights[0])):
				weights[i][j] = round(weights[i][j], 2)
		#self.weight_text.set(str(weights))
		self.weight_text.insert('end', str(weights))
		...
		if len(x_train[0, :]) > 2:
			self.train_ax.scatter(x_train[:, 0], x_train[:, 1], x_train[:, 2], c=y_train)
		else:
			self.train_ax.scatter(x_train[:, 0], x_train[:, 1], c=y_train)
		# draw surface
		for i in range(len(weights)):
			x = np.linspace(np.min(x_train[:, 0]), np.max(x_train[:, 0]), 100)
			y = np.linspace(np.min(x_train[:, 0]), np.max(x_train[:, 0]), 100)
			X, Y = np.meshgrid(x, y)
			z = np.real((-weights[i][2] * Y - weights[i][1] * X - weights[i][0])/weights[i][3])
			self.train_ax.plot_surface(X, Y, z)
		self.train_plt.draw()
		...
		if len(x_test[0, :]) > 2:
			self.test_ax.scatter(x_test[:, 0], x_test[:, 1], x_test[:, 2], c=y_test)
		else:
			self.test_ax.scatter(x_test[:, 0], x_test[:, 1], c=y_test)
		# draw surface
		for i in range(len(weights)):
			x = np.linspace(np.min(x_test[:, 0]), np.max(x_test[:, 0]), 100)
			y = np.linspace(np.min(x_test[:, 0]), np.max(x_test[:, 0]), 100)
			X, Y = np.meshgrid(x, y)
			z = np.real((-weights[i][2] * Y - weights[i][1] * X - weights[i][0])/weights[i][3])
			self.test_ax.plot_surface(X, Y, z)
		self.test_plt.draw()
